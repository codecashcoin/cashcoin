...
//                                                            based on official blockchain technology
...
#if defined(NDEBUG)
# error "Cashcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x87173ac952d2296b44aee69eba6e6ebf751a5384ff7fe816372a5183a4bbf601");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cashcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cashcoin Signed Message:\n";
...
{
    // Cashcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Cashcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Cashcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Cashcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cashcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cashcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is Cashcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
ParseHex("153784710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9")<< OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522220360;
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        try {
6            unsigned int nBlockSize = ::GetSerializeSize(block, SER_DISK, CLIENT_VERSION);
            CDiskBlockPos blockPos;
            CValidationState state;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Cashcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CashcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CashcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CashcoinMiner(CWallet *pwallet)
{
    printf("CashcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CashcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CashcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CashcoinMiner, pwallet));
}
